...
// Method definitions for TestFixture::DummyIoChannelFactory (declared in test-fixture.h).
kj::Own<WorkerInterface> TestFixture::DummyIoChannelFactory::startSubrequest(
    uint channel, SubrequestMetadata metadata) {
  KJ_FAIL_ASSERT("no subrequests");
}
kj::Own<IoChannelFactory::SubrequestChannel> TestFixture::DummyIoChannelFactory::
    getSubrequestChannel(
        uint channel, kj::Maybe<Frankenvalue> props, kj::Maybe<VersionRequest> versionRequest) {
  KJ_FAIL_ASSERT("no subrequests");
}
capnp::Capability::Client TestFixture::DummyIoChannelFactory::getCapability(uint channel) {
...
TimerChannel& TestFixture::DummyIoChannelFactory::getTimer() {
  return timer;
}
kj::Promise<void> TestFixture::DummyIoChannelFactory::writeLogfwdr(
    uint channel, kj::FunctionParam<void(capnp::AnyPointer::Builder)> buildMessage) {
  KJ_FAIL_ASSERT("no log channels");
}
kj::Own<IoChannelFactory::ActorChannel> TestFixture::DummyIoChannelFactory::getGlobalActor(
    uint channel,
...
kj::Own<IoChannelFactory::ActorChannel> TestFixture::DummyIoChannelFactory::getColoLocalActor(
